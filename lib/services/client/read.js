...
function cancelObservation(objUri, resourceId, callback) {
    if (observers[objUri + '/' + resourceId]) {
        ...
        debug('Tried to remove an unexistant observation on obj URI [%s] and resourceId [%s]',
            objUri, resourceId);
    }
...
function ResourceObserver(stream, resourceId, storedObject) {
    var data = {
        ...
        listener: function newModification(method, id, value) {
            if (method === 'setResource' && resourceId === id.toString()) {
                debug('Sending data update to the server for object [%s]', data.uri);

                try {
                    stream.setOption('Observe', data.observation);
                    stream._packet.confirmable = false;
                    stream.write(value.toString());
                    data.observation++;
                    data.lastObservation = Date.now();
                } catch(error) {
                    debug('Tried to write to a closed stream. Cleaning the handler.');
                    cancelObservation(data.uri, resourceId, function() {
                        debug('Observation of [%s] cancelled', data.id);
                    });
                }
            }
        }
    ...
    function getLastMeasure() {
        objectRegistry.get(storedObject.objectUri, function (error, obj) {
            if (error) {
                debug('Error retrieving the last resource value');
            } else {
    ...
    objectRegistry.getAttributes(storedObject.objectUri + '/' + resourceId, function(error, attributes) {
        if (error) {
            debug('Couldn\'t find attributes for the [%s] resource URI. Falling back to value changed');
        }
        ...
        if (attributes || config.client.observe.period) {
            ...
            if ( period) {
                debug('Creating a scheduled observation for [%d] miliseconds for object [%s]',
            ...
            debug('Creating an onchange subscription for object [%s]', storedObject.objectUri);
...
function handleRead(req, res, handler) {
    debug('Handling incoming read request');
    ...
    extractUriInfo(req, res, function (error, objectUri, resourceId) {
        if (error) {
            ...
            if (_.some(req.options, _.matches({name: 'Accept', value: 'application/link-format'}))) {
                debug('Retrieving simple response with static information for object: %s', objectUri);
                if (resourceId) {
                ...
                debug('Creating observers for object URI: %s', objectUri);
                async.waterfall([
                    apply(objectRegistry.get, objectUri),
