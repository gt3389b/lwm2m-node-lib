...
var async = require('async'),
    debug = require('../../debug')('unregistration'),
    registry;
...
function endUnregistration(req, res) {
    return function (error, result) {
        if (error) {
            ...
            debug('Unregistration request ended successfully');
...
function applyHandler(handler, removedObj, callback) {
    debug('Calling user handler for unregistration actions for device [%s]', removedObj.name);
