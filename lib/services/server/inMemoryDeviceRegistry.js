...
/**
 * Gets the device that has the device name passed as a parameter (should be unique) at the address specified, or return a DeviceNotFound error
 * in case none exist.
 *
 * @param {String} deviceName       Name of the device to retrieve.
 */
function getByNameAddress(deviceName, address, callback) {
    var result;

    for(var key in registry) {
        if (registry[key] && registry[key].name === deviceName && registry[key].address == address) {
            result = registry[key];
        }
    }
...
function register(object, callback) {
    ...
    getByNameAddress(object.name, object.address, function(error, result){
        if (!error && result) {
