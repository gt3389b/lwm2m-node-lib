...
/**
 * Gets the device that has the device address passed as a parameter (should be unique) or return a DeviceNotFound error
 * in case none exist.
 *
 * @param {String} deviceAddress      Remote endpoint
 */
function getByAddress(deviceAddress, callback) {
    var result;

    for(var key in registry) {
        if (registry[key] && registry[key].address+":"+registry[key].port === deviceAddress) {
            result = registry[key];
        }
    }
    ...
    if (result) {
        ...
        callback(new errors.DeviceNotFound(deviceAddress));
    }
}
...
function register(object, callback) {
    var address = object.address+":"+object.port;
    function save(existingId, cb) {
        var id = 0;
        if (existingId)
        {
            id = existingId;
    ...
    getByAddress(address, function(error, result){
        if (!error && result) {
            return save(result.id, callback);
        }
        else {
            return save(null, callback);
        }
    });
}
