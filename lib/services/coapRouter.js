...
var libcoap = require('coap'),
    debug = require('../debug')('coapRouter'),
...
function startCoap(config, callback) {
    ...
    debug('Starting COAP Server on port [%d]', config.port);
    ...
    serverInfo.server.on('error', function(error) {
        debug('An error occurred creating COAP listener: %j', error);
        callback(error);
    });

    serverInfo.server.listen(config.port, function (error) {
        if (error) {
            debug('Couldn\'t start COAP server: %s', error);
        } else {
...
function stopCoap(serverInfo, callback) {
    debug('Stopping COAP Server');

    if (serverInfo.server) {
        serverInfo.server.close(callback);
    } else {
        debug('Tried to close an unexistent server');
        callback();
    }
}
