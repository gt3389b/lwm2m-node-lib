...
function validateTypes(config, callback) {
    ...
    debug('Validating configuration types');
...
function start(serverConfig, startCallback) {
    ...
    debug('Starting Lightweight M2M Server');

    if (config.deviceRegistry && config.deviceRegistry.type === 'mongodb') {
        debug('Mongo DB Device registry selected for Lightweight M2M Library');
        registry = require('./services/server/mongodbDeviceRegistry');
    } else {
        debug('Memory Device registry selected for Lightweight M2M Library');
