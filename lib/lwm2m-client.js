...
function createRegisterResponseHandler(deviceInformation, callback) {
    return function responseHandler(res) {
        if (res.code.match(/^2\.\d\d$/)) {
            logger.info(context, 'Registration operation succeeded to host [%s] and port [%s] with code [%s]',
...
function register(host, port, url, endpointName, callback) {
    ...
    var agent = new coap.Agent({type: config.client.ipProtocol});
    agent.setRequestOptions({
        dtls: config.client.dtls,
        privateKey: config.client.privateKey,
        certificate: config.client.certificate
    });
    var req = agent.request(creationRequest);
